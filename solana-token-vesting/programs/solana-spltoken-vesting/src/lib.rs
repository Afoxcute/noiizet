...
declare_id!("FPMWFeVjrYcZJXRDU8W2XrMVNHmHGa6qXrJ16rW92fRp");
...
#[program]
pub mod solana_spltoken_vesting {
    use super::*;
    ...
    pub fn vest(_ctx: Context<Vest>, amount: u64) -> Result<()> {
        msg!("vest_start");
        let state = &mut _ctx.accounts.state;
        let user = &mut _ctx.accounts.user;
        let pool = &mut _ctx.accounts.pool;
        ...
        //     .unwrap();
        user.last_vest_time = _ctx.accounts.clock.unix_timestamp;
    ...
    pub fn unvest(_ctx: Context<Vest>, amount: u64) -> Result<()> {
        msg!("unvest_start");
        let state = &mut _ctx.accounts.state;
        let user = &mut _ctx.accounts.user;
        let pool = &mut _ctx.accounts.pool;

        require!(user.amount >= amount, ErrorCode::UnvestOverAmount);
        require!(
            user.last_vest_time
        ...
        let seconds = _ctx
            ...
            .clock
            .unix_timestamp
            .checked_sub(user.last_vest_time)
            .unwrap();
